...
local editor_highlights = require("marlyn.groups.editor")
local syntax_highlights = require("marlyn.groups.syntax")
local terminal = require("marlyn.terminal")
...
function M.setup()
	...
	for integration, enabled in pairs(config.integrations) do
		if enabled then
			local highlights = require("marlyn.groups.integrations." .. integration)
			if highlights then
